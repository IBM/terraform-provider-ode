...
func TestNewAndHeader(t *testing.T) {
	...
	if _, err = New(context.Background(), "GET", ":", nil); !errors.Is(err, ErrInvalidURL) {
		t.Fatalf("want: ErrInvalidURL, got %v", err)
	}
	if _, err = New(context.Background(), "GET", "https://ex", Header("\n", "")); !errors.Is(err, ErrInvalidParam) {
		t.Fatalf("expected ErrInvalidParam for bad header")
	}
}

func TestQueryAndQueryMap(t *testing.T) {
	req, _ := New(context.Background(), "GET", "https://ex.com", QueryMap(map[string]string{"key": "value"}))
	if got := req.URL.Query().Get("key"); got != "value" {
		t.Fatalf("want query not encoded, got %q", got)
	}

	_, err := New(context.Background(), "GET", "https://fake.com", Query(url.Values{"": {"fake"}}))
...
func TestJSONOpt(t *testing.T) {
	req, _ := New(
		context.Background(), "POST", "https://fake",
		JSON(P{A: "body"}),
	)
...
func TestFormOpt(t *testing.T) {
	req, _ := New(context.Background(), "POST", "https://fake", Form(map[string]string{"foo": "bar"}))
...
func TestMultipartSuccess(t *testing.T) {
	req, err := New(
		context.Background(), "POST", "http://fake",
		Multipart(
...
func TestHeaderSanitisesCRLF(t *testing.T) {
	req, _ := New(
		context.Background(), "GET", "https://fake.com",
		Header("X-Kyle", "good\r\nnbad"),
	)
...
func TestContentLengthComputed(t *testing.T) {
	...
	req, _ := New(context.Background(), "POST", "https://fake", JSON(payload))
...
func TestQueryHelperInternalError(t *testing.T) {
	v := url.Values{"": {"a"}}
	_, err := New(context.Background(), "GET", "https://fake", Query(v))
	if !errors.Is(err, ErrInvalidParam) {
...
func TestNewInvalidMethod(t *testing.T) {
	_, err := New(context.Background(), "not a method", "https://fake", nil)
	if err == nil {
