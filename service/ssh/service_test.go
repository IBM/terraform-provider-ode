	...
	mux.HandleFunc(
		"/", func(w http.ResponseWriter, r *http.Request) {
			...
			case v2Path:
				...
				if err != nil {
					t.Fatalf("failure to write mockToken")
				}
			case v1Path:
				http.Error(w, "forbidden", http.StatusForbidden)
			case keyPath:
				w.Header().Set("Content-Type", "application/json")
				_, err := w.Write([]byte(`{"token":"fakeToken"}`))
				if err != nil {
					t.Fatalf("failure to write fakeToken")
				}
			case v2keyPath:
				w.Header().Set("Content-Type", "application/json")
				_, err := w.Write([]byte(`{"token":"fakeToken"}`))
...
func TestService(t *testing.T) {
	...
	t.Run(
		"Instance403", func(t *testing.T) {
			body.Reset()
			_, err := svc.AuthenticateInstance(t.Context(), "uuid", "u", "p")
			if err == nil {
	...
	t.Run(
		"KeyMultipartOK", func(t *testing.T) {
			...
			tok, err := svc.AuthenticateKey(t.Context(), "uuid", "u", "p", []byte(keyData))
	...
	t.Run(
		"KeyMultipartOK", func(t *testing.T) {
			...
			tok, err := svc.AuthenticateTargetWithKey(t.Context(), "h", "u", "pass", 22, []byte(keyData))
			if err != nil || tok.Value != "fakeToken" {
				t.Errorf("got: %v, %q, want: nil, %q", err, tok.Value, "k")
			}
			if *path != v2keyPath {
